...
import configs.xd as xd_option_pseudo
...
def test(model,full_path, testdataloader, maxlen, prompt_text, gt, gtsegments, gtlabels, device):
    ...
    ap1 = ap1.tolist()
    ap2 = ap2.tolist()
    print("ap1:",len(np.repeat(ap1, 16)))
    print("gt:",len(gt))
...
if __name__ == '__main__':
    ...
    args = xd_option_pseudo.parser.parse_args()
    full_path ="../exp_xd"
    ...
    model_param = torch.load('ckpts/xd.pth', weights_only=False)['model_state_dict']
    model.load_state_dict(model_param)
    test(model, full_path, test_loader, args.visual_length, prompt_text, gt, gtsegments, gtlabels, device)