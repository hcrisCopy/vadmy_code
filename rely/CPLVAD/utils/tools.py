...
def get_batch_label(texts, prompt_text, label_map: dict, type="prompt"):
    # label one-hot encoding (fine level)
    ...
    if len(label_map) != 7:           
        if len(label_map) == 2:
            ...
            prompt_labels = ["normal", "abuse", "arrest", "arson", "assault", "burglary", "explosion", "fighting", "roadAccidents", "robbery", "shooting", "shoplifting", "stealing", "vandalism"]
            for text in texts:
                label_vector = torch.zeros(len(prompt_text))
                if text in label_map:
                    label_text = label_map[text]
                    label_vector[prompt_labels.index(label_text)] = 1

                label_vector = label_vector.unsqueeze(0)
                label_vectors = torch.cat([label_vectors, label_vector], dim=0)
    # XD dataset
    else:
        prompt_labels = ["normal", "fighting", "shooting", "riot", "abuse", "car accident", "explosion"]
        
        for text in texts:
            label_vector = torch.zeros(len(prompt_labels))
            labels = text.split('-')
            for label in labels:
...
def get_prompt_text(label_map: dict, type = "prompt"):
    prompt_text = []
    if type == "word":
        ...
        for v in label_map.values():
            template = "A video of the "
            prompt_text.append(template+v)
    return prompt_text
...
# Split CLIP features to fixed length
