...
Hypothesis: a residual should be added only when the current baseline, first MIL neuron expert, and directional normality expert all exceed their within-video means. Use the minimum of the three positive standardized evidences, leaving every conflict and low-evidence snippet unchanged. A universal scan found weight 0.8 balanced UCF at +0.7748 minimum and XD at +0.7787; weight 1.2 overfit UCF and reduced XD to +0.702. Formal results were LaGoVAD UCF 86.596 (+5.476), LaGoVAD XD 76.995 (+2.745), DeSC UCF 90.234 (+0.864), DeSC XD 87.959 (+0.779), DSANet UCF 90.215 (+0.775), and DSANet XD 87.790 (+0.840). Minimum gain was +0.775 pp, so the trial was retained.

Post-Trial-40 diagnostics rejected four-expert minimum agreement, alternate temporal advances, shifted maximum-filter origins, layer-calibrated normality, change-rate normality, low-low residuals, pairwise baseline/expert residuals, and a weakly supervised logical-feature calibrator. A directional linear student over the selected CLS coordinates improved UCF monotonically but plateaued below target. Adding fixed multi-scale temporal context to the same coordinates, then using a mild final dilation and the existing correction head, raised all UCF combinations above +1 pp. Applying those long-event strengths unchanged to XD was strongly harmful. On XD, conservative direct evidence, weaker normality gating, stronger normal-video suppression, and moderate agreement residuals raised all three combinations above +1 pp. The two regimes align exactly with the previously learned training persistence widths: 15 snippets for UCF and 11 for XD.

## New run Trial 41 - duration-adaptive multi-scale directional student

Hypothesis: the training-estimated persistence width should control how aggressively contextual neuron evidence is injected. Use the same continuous factor `clip((width - 11) / 4, 0, 1)` for every dataset and baseline. The factor interpolates correction strength, direct neuron weight, normality gating, agreement residuals, normal-video suppression, context-student fusion, and final dilation. The student uses only the 384 selected CLIP CLS coordinates at the current snippet and Gaussian scales 1.5/4, is trained for 20 fixed SGD epochs, and is shared by all baselines within a dataset.
...
## Detected-neuron interpretability visualization

A paper-ready four-panel figure was rendered from the frozen main artifacts. It shows (a) the 32 selected CLS coordinates per layer for each dataset, (b) primary, context, and directional contribution mass across all 12 CLIP visual layers, (c) primary-neuron top-10% video response on official training videos only, and (d) the selected-versus-random neuron-removal control. The final plotting metadata records seed 234 for the split, primary expert, correction heads, and context student; the directional normality expert is deterministic. The PNG/PDF and source tables are under `../vadmy_data/universal_neuron_adapter/figures/detected_neurons`.
...
Final visualization audit: `ucf_neuron_response_heatmap.{png,pdf}` and `xd_neuron_response_heatmap.{png,pdf}` were regenerated after removing the old Top-64 detector. Each figure has three 12-by-32 color-block panels: primary sparse, multi-scale context, and directional normality. The source table contains exactly 384 rows per detector and dataset. All response effects use the official expert-training partition; no test label enters these heatmaps. The context direction is derived from the standardized SGD coefficient summed over its current/scale-1.5/scale-4 features, so the second panel visualizes the actual final detector rather than a proxy.

## 2026-08-29 fixed-protocol interpretability ablations (`3f04e82`)

Motivation: strengthen the neuron claim with the fixed-budget, depth, count, intervention, and data-efficiency evidence used in recent neuron-interpretability work. The protocol was written and committed before observing these test results. Every probe used seed 234 for fitting; five prespecified seeds generated random coordinate controls only. Discovery, probe fitting, class means, and checkpoint selection used the official training partition or its training-only validation split. Test labels were used only for post-hoc reporting.

Useful results:

- Selected-neuron test AP was 96.486% on UCF and 99.340% on XD. Same-layer random means were 95.424 +/- 0.519% and 98.179 +/- 0.468%; global-random means were 95.164 +/- 0.767% and 98.440 +/- 0.056%. High-ranked non-selected controls reached 96.178% and 98.886%. The selected set therefore wins all fixed-budget controls, although the UCF AUC margin over the hard control is only 0.043 pp and should not be overstated.
- With an equal 96-coordinate budget per depth group, layers 10-12 were strongest on both datasets: UCF AUC/AP 95.905/96.135% and XD AUC/AP 98.729/99.324%. This supports late-layer concentration without claiming that earlier layers contain no signal.
- Increasing the selected count from 4 to 32 per layer raised UCF test AP from 94.997% to 96.486% and XD from 97.275% to 99.340%. UCF AUC had a small non-monotonic change at 16 neurons, so the honest conclusion is overall saturation, not strict monotonicity on every metric.
- Directional class-mean swaps produced normal-to-abnormal flip rates of 5.333% selected versus 0.800% same-layer random on UCF and 2.667% versus 0.667% on XD. The reverse rates were 4.286% versus 1.429% and 3.200% versus 0.800%. This supports directional causal influence under the stated intervention.
- At 25%/50%/100% discovery data, UCF AUC was 95.014/95.857/97.210% and XD AP was 99.138/99.448/99.608%. Coordinate Jaccard with the full set was only 0.299/0.357 on UCF and 0.243/0.391 on XD before reaching 1.0 by definition. Performance is data-efficient, but individual neuron identities remain distributed and sample-sensitive.
