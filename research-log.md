...
A paper-ready four-panel figure was rendered from the frozen main artifacts. It shows (a) the 32 selected CLS coordinates per layer for each dataset, (b) learned and directional contribution mass across all 12 CLIP visual layers, (c) primary-neuron top-10% video response on official training videos only, and (d) the selected-versus-random neuron-removal control. The plotting metadata records the composite seed policy: split, primary expert, and correction heads use seed 234; the diverse expert and context student use seed 3407; the directional normality expert is deterministic. The PNG/PDF and source tables are under `../vadmy_data/universal_neuron_adapter/figures/detected_neurons`.
...
The coarse dimension-density view was superseded because its sparse percentages could be mistaken for weak activation. The final main-paper visualization uses one figure per dataset and shows only the neurons actually retained by each detector: Primary sparse Top-32, diverse sparse Top-64, and directional normality Top-32 for every layer. Cells are ordered by raw selection importance and normalized only by the strongest selected neuron in the same layer; this improves readability without altering model values or implying cross-detector magnitude comparability. The exported source table preserves every layer, rank, original dimension, raw importance, and displayed normalized value. The exact coordinate heatmap remains a supplementary diagnostic.
