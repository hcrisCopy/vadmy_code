...
from responsibility_cross_expert.losses import (
    binary_topk_mil,
...
class BaselineAdapter(nn.Module):
    ...
    def binary_training_logits(self, output: BaselineOutput) -> list[torch.Tensor]:
        """Return independent binary streams receiving dense supervision."""
...
class DSANetAdapter(BaselineAdapter):
    ...
    def set_train_scope(self, scope: str) -> None:
        ...
        if scope == "frozen":
            return
        if scope == "binary_head":
            self.base.classifier.requires_grad_(True)
        if scope not in {"frozen", "binary_head"}:
...
class DeSCAdapter(BaselineAdapter):
    ...
    def binary_training_logits(self, output: BaselineOutput) -> list[torch.Tensor]:
        return [stream[1].squeeze(-1) for stream in output.raw]
...
class LaGoVADAdapter(BaselineAdapter):
    ...
    def set_train_scope(self, scope: str) -> None:
        ...
        if scope == "frozen":
            return
        if scope == "binary_head":
            self.base.bin_head.requires_grad_(True)
        if scope not in {"frozen", "binary_head"}:
            raise ValueError(f"unknown train scope: {scope}")
