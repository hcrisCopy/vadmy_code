# DeSC + 主方法（Universal CLS-neuron Adapter）运行说明
...
cp baseline/DeSC/list/gt_ucf.npy ../vadmy_data/annotations/ucf/gt.npy
cp baseline/DeSC/list/gt.npy     ../vadmy_data/annotations/xd/gt.npy
```

- 权重已就位（DeSC 双流：sensitivity + consistency）：
  `../vadmy_data/model/DeSC/{ucf,xd}_sensitivity_stream.pth`、`{ucf,xd}_consistency_stream.pth`。
- 本次运行输出根目录（下文统一用 `$OUT`）：
...
## 5. 缓存 DeSC 冻结分数（本 baseline 专属）

DeSC 是双流模型，需要同时指定 sensitivity 与 consistency 两份官方权重。推理协议（UCF 滑窗 TTA / XD 插值到 256 再插回）已在 `desc_inference` 中按作者发布版实现，训练用 `train_all.csv` 覆盖 train/val 两份 key，测试用 `test.csv`：

```bash
for dataset in ucf xd; do
  base="$OUT/$dataset/desc"
  python -m universal_neuron_adapter.cache_baseline \
    --baseline desc --baseline-root baseline/DeSC \
    --sensitivity-weight "../vadmy_data/model/DeSC/${dataset}_sensitivity_stream.pth" \
    --consistency-weight "../vadmy_data/model/DeSC/${dataset}_consistency_stream.pth" \
...
输出：`$OUT/<dataset>/desc/baseline_{train,test}/baseline_scores.csv`（含 `baseline_score_path` 与 `semantic_score_path`）。
...
for dataset in ucf xd; do
  ...
  python -m universal_neuron_adapter.train_correction \
    ...
    --baseline desc --dataset "$dataset" --out-dir "$base/correction" \
    --width 32 --max-epoch 15 --batch-size 32 --lr 0.0003 --weight-decay 0.0001 \
    --maximum-length 256 --num-workers 4 --seed 234 --device cuda --resume
done
```

输出：`$OUT/<dataset>/desc/correction/model_best.pth`（按训练集划分出的验证集选点）。
...
for dataset in ucf xd; do
  ...
  python -m universal_neuron_adapter.evaluate \
    ...
    --baseline desc --dataset "$dataset" --out-dir "$base/evaluation" \
...
输出：`$OUT/<dataset>/desc/evaluation/metrics.json`（baseline 与 corrected 的 AUC/AP）+ `per_video.csv` + `curves/`。

> 注：DeSC 发布版评测（`ucf/xd_test_tta.py`）不包含检测 mAP 协议，因此本 md 不含第 8 步（与 DSANet/VadCLIP 的官方 mAP 协议不同，不强行套用）。
...
| DeSC 冻结分数 | `runs/<hash>/<dataset>/desc/baseline_{train,test}/` |
| 校正头 | `runs/<hash>/<dataset>/desc/correction/` |
| 正式指标 | `runs/<hash>/<dataset>/desc/evaluation/metrics.json` |
