# VadCLIP + 主方法（Universal CLS-neuron Adapter）运行说明
...
cp baseline/VadCLIP/list/gt_ucf.npy ../vadmy_data/annotations/ucf/gt.npy
cp baseline/VadCLIP/list/gt.npy     ../vadmy_data/annotations/xd/gt.npy
```

- 权重已就位：`../vadmy_data/model/VadClip/model_ucf.pth`、`model_xd.pth`。
- 本次运行输出根目录（下文统一用 `$OUT`）：
...
## 5. 缓存 VadCLIP 冻结分数（本 baseline 专属）

用官方权重跑冻结推理，得到每条视频的 binary + semantic 概率曲线（训练用 `train_all.csv` 覆盖 train/val 两份 key，测试用 `test.csv`）：

```bash
for dataset in ucf xd; do
  base="$OUT/$dataset/vadclip"
  python -m universal_neuron_adapter.cache_baseline \
    --baseline vadclip --baseline-root baseline/VadCLIP \
    --baseline-weight "../vadmy_data/model/VadClip/model_${dataset}.pth" \
...
输出：`$OUT/<dataset>/vadclip/baseline_{train,test}/baseline_scores.csv`（含 `baseline_score_path` 与 `semantic_score_path`）。
...
for dataset in ucf xd; do
  ...
  python -m universal_neuron_adapter.train_correction \
    ...
    --baseline vadclip --dataset "$dataset" --out-dir "$base/correction" \
    --width 32 --max-epoch 15 --batch-size 32 --lr 0.0003 --weight-decay 0.0001 \
    --maximum-length 256 --num-workers 4 --seed 234 --device cuda --resume
done
```

输出：`$OUT/<dataset>/vadclip/correction/model_best.pth`（按训练集划分出的验证集选点）。
...
for dataset in ucf xd; do
  ...
  python -m universal_neuron_adapter.evaluate \
    ...
    --baseline vadclip --dataset "$dataset" --out-dir "$base/evaluation" \
...
输出：`$OUT/<dataset>/vadclip/evaluation/metrics.json`（baseline 与 corrected 的 AUC/AP）+ `per_video.csv` + `curves/`。

## 8.（可选）官方检测 mAP（VadCLIP 自带官方 `ucf/xd_detectionMAP.py`）

```bash
for dataset in ucf xd; do
  base="$OUT/$dataset/vadclip"
  if [[ "$dataset" == "ucf" ]]; then
    segments=baseline/VadCLIP/list/gt_segment_ucf.npy
    labels=baseline/VadCLIP/list/gt_label_ucf.npy
  else
    segments=baseline/VadCLIP/list/gt_segment.npy
    labels=baseline/VadCLIP/list/gt_label.npy
  fi
  cache="$OUT/detection_map/$dataset/vadclip_cache"
  python -m universal_neuron_adapter.cache_baseline \
    --baseline vadclip --baseline-root baseline/VadCLIP \
    --baseline-weight "../vadmy_data/model/VadClip/model_${dataset}.pth" \
    --dataset "$dataset" --manifest "$OUT/$dataset/data/test.csv" --split test \
  ...
  python -m universal_neuron_adapter.evaluate_detection_map \
    ...
    --segment-gt "$segments" --label-gt "$labels" --baseline-root baseline/VadCLIP \
...
- 一键脚本只运行 VADCLIP 的 UCF 与 XD 两项实验；默认实验名为 `formal_seed234`，可通过 `EXPERIMENT_NAME` 修改。
- `aggregate_metric` 现在**自动跳过还没跑的 baseline**（只汇总已存在的指标并打印 `[skip]` 提示），不会再因为缺某几个结果而崩溃。
- 同名实验会断点续跑并复用数据集级神经元专家；不同代码版本或 seed 禁止混入同一目录。

输出：`$OUT/summary.json`（相对 VadCLIP 论文基线 88.02% AUC / 84.51% AP 的增益）。
...
```bash
bash run_instructions/run_vadclip.sh
```
...
| VadCLIP 冻结分数 | `runs/<hash>/<dataset>/vadclip/baseline_{train,test}/` |
| 校正头 | `runs/<hash>/<dataset>/vadclip/correction/` |
| 正式指标 | `runs/<hash>/<dataset>/vadclip/evaluation/metrics.json` |
