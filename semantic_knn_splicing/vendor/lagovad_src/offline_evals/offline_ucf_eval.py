...
from src.datasets.UCFCrime import UCFCrimeDataset
...
full_metric = torchmetrics.Accuracy('multiclass', num_classes=14)
f1_metric = torchmetrics.F1Score('multiclass', num_classes=14, average='macro')
dataset = UCFCrimeDataset(
    '/data/datasets/PreVAD',
    '/data/datasets/PreVAD/other_datasets/ucf_test_anno.json',
    vis_max_len=768,
)
for i in tqdm(range(len(dataset))):
    ds_item, pred = dataset[i], preds[i]
    probs = torch.softmax(torch.tensor(pred['real_mul_scores']).mean(-1) / 5.0, dim=0)
