...
OUT="../vadmy_data/shift_residual_head_tuning/${DATASET}/${BASELINE}"
...
python -m shift_residual_head_tuning.train \
  --baseline "$BASELINE" \
  --baseline-root "$BASELINE_ROOT" \
  "${WEIGHT_ARGS[@]}" \
  --dataset "$DATASET" \
...
python -m shift_residual_head_tuning.evaluate \
  --baseline "$BASELINE" \
  --baseline-root "$BASELINE_ROOT" \
  "${WEIGHT_ARGS[@]}" \
  --dataset "$DATASET" \
...
python -m shift_residual_head_tuning.visualize_diagnostics \
