#!/usr/bin/env python3
"""Evaluate U-shaped dual injection with tuned binary score heads."""
...
from shift_u_dual_head_tuning.heads import load_score_head_state
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate Shift U-dual plus score-head tuning.")
    ...
    with torch.no_grad():
        for key_value, group in tqdm(frame.groupby("key", sort=False), desc="evaluate U-dual+head videos", unit="video"):
            key = str(key_value)
