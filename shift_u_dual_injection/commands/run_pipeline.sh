...
# These artifacts are byte-for-byte shared with the earlier single-injection
# control.  Creating them here when absent does not run that control's trainer.
SHARED="../vadmy_data/shift_residual_head_tuning/${DATASET}/${BASELINE}"
OUT="../vadmy_data/shift_u_dual_injection/${DATASET}/${BASELINE}"
...
python -m shift_u_dual_injection.train \
  --baseline "$BASELINE" \
  --baseline-root "$BASELINE_ROOT" \
  "${WEIGHT_ARGS[@]}" \
  --dataset "$DATASET" \
...
python -m shift_u_dual_injection.evaluate \
  --baseline "$BASELINE" \
  --baseline-root "$BASELINE_ROOT" \
  "${WEIGHT_ARGS[@]}" \
  --dataset "$DATASET" \
...
python -m shift_u_dual_injection.visualize_diagnostics \
