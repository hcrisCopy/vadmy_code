#!/usr/bin/env python3
"""Official-style evaluation for frozen-baseline U-shaped dual injection."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate frozen-baseline Shift U-shaped dual injection.")
    ...
    if checkpoint.get("method") != UDualInjector.method_name:
    ...
    injector = UDualInjector.from_config(checkpoint["injector_config"]).to(device)
    ...
    with torch.no_grad():
        for key_value, group in tqdm(frame.groupby("key", sort=False), desc="evaluate U-dual videos", unit="video"):
            key = str(key_value)
    ...
    metrics = {}
    for name, pieces in (("binary_score", binary_all), ("official_refined", refined_all)):
