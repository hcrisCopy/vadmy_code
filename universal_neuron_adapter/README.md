...
The adapter combines a learned score correction with three sparse CLS-neuron views. Training-inferred persistent events use a stronger correction contribution, while short events retain the baseline anchor. A direct residual recovers baseline misses where the primary and directional detectors have positive intersecting evidence. Short events use a fixed conservative residual; persistent events use a stronger residual multiplied by the Sigmoid of the smaller logit from two training-only video priors. The same duration factor continuously interpolates these strengths. Baseline-neuron agreement separately controls local event expansion, and persistent events fuse a second temporal scale of width `2w-1`.
