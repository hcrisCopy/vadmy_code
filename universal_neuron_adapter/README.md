...
The current hypothesis conservatively combines the frozen baseline logit, the first-round learned neuron-correction logit, and three CLS-neuron experts. Two MIL experts use 32 and 64 active dimensions per CLIP layer. A third baseline-independent normality expert estimates neuron distributions from normal training snippets and selects dimensions that show stable abnormal-video deviation. The experts jointly gate event propagation. A joint current-baseline/CLS classifier suppresses only high-confidence normal videos, fixed median and Gaussian filters remove isolated jitter, and scores are advanced one snippet within each video. The same features, models, and coefficients are used for every baseline and dataset. All six evaluations use strict 16-frame snippet expansion and write only below `../vadmy_data/universal_neuron_adapter`.
