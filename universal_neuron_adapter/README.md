...
The adapter combines a conservative learned score correction with three sparse CLS-neuron views. Agreement gates permit local event expansion only where baseline and neuron evidence support an anomaly. Two training-only video priors suppress a video only when both independently support normality; their equally weighted log-odds form a product-of-experts consensus. The second prior uses all three neuron views and their pairwise correlations and disagreements. The temporal width is inferred from training-video evidence and controls persistence smoothing and dilation through one shared continuous rule. The formula and coefficients are identical for all six baseline/dataset settings.
