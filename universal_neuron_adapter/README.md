...
The adapter combines a conservative learned score correction with three sparse CLS-neuron views. Positive intersection evidence from the primary and directional detectors forms a bounded residual, while their joint negative evidence suppresses likely false positives. A multi-scale context student is trained only from high-confidence snippets where the primary and directional training detectors agree positively; it uses the shared fixed seed 234 and never consumes baseline scores. Training-only video priors and neuron persistence constrain video-level strength and temporal range.
