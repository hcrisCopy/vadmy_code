...
The adapter combines a conservative learned score correction with three sparse CLS-neuron views. A direct residual recovers baseline misses where the primary and directional detectors have positive intersecting evidence. Its strength is multiplied by a video anomaly-context gate: the Sigmoid of the smaller logit from two training-only video priors, so local neuron evidence is attenuated on likely normal videos. Baseline-neuron agreement separately controls local event expansion. The temporal width is inferred from training-video evidence; persistent-event data fuse the inferred median trajectory with a second scale of width `2w-1`, while short-event data retain the single scale. The same continuous duration factor controls this consensus and dilation for all six settings.
