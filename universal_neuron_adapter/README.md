...
The adapter combines a conservative learned score correction with three sparse CLS-neuron views. Their positive response-correlation matrix defines a spectral consensus module. The event-gate weights equally blend a stable principal-eigenvector prior averaged over official training videos and the current video's principal eigenvector. Positive and negative hard intersections remain conservative. Training-only persistence controls temporal aggregation and boundary recovery. No module consults another baseline stream.
