...
The adapter combines three distinct CLS-neuron modules. The primary Top-32 expert learns sparse activation-level evidence. A complementary dynamic Top-32 expert, trained with the same fixed seed 234, uses the same neuron definition but jointly models activation levels and adjacent-snippet changes; it is not a larger copy with another seed. The deterministic directional expert measures one-sided departure from normal training moments. A spectral consensus graph weights their event evidence, while hard intersections and persistence modules remain conservative.
