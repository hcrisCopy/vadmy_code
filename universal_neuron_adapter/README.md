...
The adapter combines a conservative learned score correction with three sparse CLS-neuron views. Their positive response-correlation matrix defines a spectral consensus module: its principal eigenvector decides which views to trust, while its normalized largest eigenvalue controls how strongly the soft event gate propagates peaks. Positive and negative hard intersections remain conservative. Training-only persistence controls temporal aggregation and boundary recovery. No module consults another baseline stream.
