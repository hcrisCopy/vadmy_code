...
The adapter combines a conservative learned score correction with three modular CLS-neuron views: a sparse MIL detector, a deterministic covariance-decorrelated consensus detector, and a directional normality detector. The consensus module uses Ledoit-Wolf precision weighting to remove redundant responses among the 32 signed training-selected coordinates in each layer, replacing the different-seed Top-64 expert. Unlike an unsigned Mahalanobis detector, it retains only the learned anomaly direction. A direct residual recovers baseline misses where the sparse and directional detectors intersect. Temporal strength is inferred from training-only neuron persistence.
