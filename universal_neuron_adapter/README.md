...
The neuron-visualization command renders a paper-ready interpretability figure, a three-detector block heatmap for the main paper, and an exact 12-layer by 768-dimension coordinate heatmap for supplementary inspection. PNG/PDF figures, source CSV tables, self-contained captions, and seed/data-provenance metadata are written under `../vadmy_data/universal_neuron_adapter/figures/detected_neurons`. Panel (c) of the interpretability figure uses training videos only; panel (d) is an explicitly labeled post-hoc neuron-removal control.
