...
The adapter combines a conservative learned score correction with three sparse CLS-neuron views. Their positive response-correlation matrix defines a spectral consensus module. Squared principal-eigenvector entries are renormalized to mean one, emphasizing the spectral-center views and reducing isolated responses. Positive and negative hard intersections remain conservative. Training-only persistence controls temporal aggregation and boundary recovery. No module consults another baseline stream.
