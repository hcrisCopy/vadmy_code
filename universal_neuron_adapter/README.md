...
The adapter combines a conservative learned score correction with three sparse CLS-neuron views. A spectral consensus graph uses cosine overlap between their positive anomaly responses; its principal eigenvector supplies mean-one detector weights, emphasizing views that activate on the same snippets and reducing isolated responses. Positive and negative hard intersections remain conservative. Training-only persistence controls temporal aggregation and boundary recovery. No module consults another baseline stream.
