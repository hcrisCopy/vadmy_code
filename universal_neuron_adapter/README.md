...
The adapter combines a conservative learned score correction with three modular CLS-neuron views. The primary sparse expert models a compact discriminative coordinate subspace. A same-budget complementary expert is constrained to the exact coordinate-orthogonal complement of the primary selection, so its additional evidence does not come from a different seed or a wider Top-K. The directional normality expert models signed departure from normal activation. Agreement gates permit local event expansion only where baseline and neuron evidence support an anomaly. The formula and coefficients are identical for all six baseline/dataset settings.
