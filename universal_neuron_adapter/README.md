...
The adapter combines a conservative learned score correction with three sparse CLS-neuron views. Their positive response-correlation matrix defines a spectral consensus module whose principal eigenvector weights the event gate. The same training-only spectral consensus selects high-confidence positive snippets for a seed-234 multi-scale context student, distilling agreement rather than one detector's noise. Positive and negative hard intersections remain conservative. No module consults another baseline stream.
