...
The adapter combines a conservative learned score correction with three sparse CLS-neuron views. A positive response-correlation graph contains the current single baseline and the three neuron detectors; the neuron entries of its principal eigenvector are renormalized to mean one and weight the soft event gate. Thus views consistent with both the baseline and other neurons gain trust, while isolated responses are reduced. Hard positive/negative intersections remain conservative. No other baseline stream is consulted.
