...
The adapter combines a conservative learned score correction with three sparse CLS-neuron views. Its spectral consensus module builds a 3×3 positive response-correlation graph in an event-sized window around every snippet; the local principal eigenvector supplies mean-one detector weights, allowing reliability to change across event phases. Positive and negative hard intersections remain conservative. Training-only persistence controls temporal aggregation and boundary recovery. No module consults another baseline stream.
