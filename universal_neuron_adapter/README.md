...
The adapter combines a conservative learned score correction with three sparse CLS-neuron views. Before fusion, each neuron evidence curve is calibrated by its label-free within-video empirical CDF and mapped to a standard-normal quantile, giving all detectors a common robust scale without test labels. A direct residual recovers baseline misses where the primary and directional detectors have positive intersecting evidence. Training-inferred persistence controls the strength of residual, context, and temporal expansion modules.
