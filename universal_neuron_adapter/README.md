...
The current hypothesis conservatively combines the frozen baseline logit, the first-round learned neuron-correction logit, and video-wise standardized neuron evidence. It then expands event peaks only where the same CLS-neuron evidence opens a soft gate. The same correction, neuron, event-width, and event-weight parameters are used for every baseline and dataset. All six evaluations use strict 16-frame snippet expansion and write only below `../vadmy_data/universal_neuron_adapter`.
