...
for dataset in ucf xd; do
  ...
  for baseline in lagovad desc dsanet; do
    ...
    cross="$OUT/$dataset/$baseline/cross_view_normalized"
    python -m universal_neuron_adapter.train_cross_view \
      --selected-manifest "$selected_root/train/selected_manifest.csv" \
      --teacher-manifest "$source_base/baseline_train/baseline_scores.csv" \
      ...
      --baseline "$baseline" --dataset "$dataset" --out-dir "$cross" \
      --width 64 --max-epoch 8 --batch-size 32 --lr 0.0003 \
    ...
    python -m universal_neuron_adapter.evaluate \
      ...
      --cross-view-model "$cross/model_best.pth" \
      ...
      --cross-view-weight 0.5 --device cuda
  done
done
