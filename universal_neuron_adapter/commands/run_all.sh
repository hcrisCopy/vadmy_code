...
for dataset in ucf xd; do
  mrmr="$ROOT/mrmr_expert_cache/$dataset/top32_penalty025_v1"
  python -m universal_neuron_adapter.fit_mrmr_expert \
    --manifest "$SOURCE/$dataset/data/expert_train.csv" --out-dir "$mrmr" \
    --active-per-layer 32 --redundancy-penalty 0.25 --maximum-length 256 --resume
  python -m universal_neuron_adapter.export_mrmr_expert \
    --manifest "$SOURCE/$dataset/data/test.csv" --expert-model "$mrmr/mrmr_expert.npz" \
    --out-dir "$mrmr/test"
  python -m universal_neuron_adapter.export_mrmr_expert \
    --manifest "$SOURCE/$dataset/data/expert_train.csv" --expert-model "$mrmr/mrmr_expert.npz" \
    --out-dir "$mrmr/train"
  ...
  for baseline in lagovad desc dsanet; do
    ...
    python -m universal_neuron_adapter.evaluate \
      ...
      --expert2-manifest "$mrmr/test/expert2_scores.csv" \
      --expert2-train-manifest "$mrmr/train/expert2_scores.csv" \
