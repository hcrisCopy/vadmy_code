...
for dataset in ucf xd; do
  temporal="$ROOT/temporal_information_cache/$dataset/top32_logdet_$RUN_KEY"
  python -m universal_neuron_adapter.fit_temporal_information_expert \
    --manifest "$SOURCE/$dataset/data/expert_train.csv" --out-dir "$temporal" \
    --active-per-layer 32 --candidate-per-layer 128 --scales 1 2 4 \
    --maximum-length 256 --resume
  python -m universal_neuron_adapter.export_temporal_information_expert \
    --manifest "$SOURCE/$dataset/data/test.csv" \
    --expert-model "$temporal/temporal_information_expert.npz" --out-dir "$temporal/test"
  python -m universal_neuron_adapter.export_temporal_information_expert \
    --manifest "$SOURCE/$dataset/data/expert_train.csv" \
    --expert-model "$temporal/temporal_information_expert.npz" --out-dir "$temporal/train"
  ...
  for baseline in lagovad desc dsanet; do
    ...
    python -m universal_neuron_adapter.evaluate \
      ...
      --expert2-manifest "$temporal/test/expert2_scores.csv" \
      --expert2-train-manifest "$temporal/train/expert2_scores.csv" \
