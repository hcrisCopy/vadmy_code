...
for dataset in ucf xd; do
  ...
  covariance="$ROOT/covariance_expert_cache/$dataset/top32_directional_v2"
  python -m universal_neuron_adapter.fit_covariance_expert \
    --manifest "$SOURCE/$dataset/data/expert_train.csv" \
    --normality-model "$normality/normality_expert.npz" \
    --out-dir "$covariance" --maximum-length 256 --resume
  python -m universal_neuron_adapter.export_covariance_expert \
    --manifest "$SOURCE/$dataset/data/test.csv" \
    --expert-model "$covariance/covariance_expert.npz" \
    --normality-model "$normality/normality_expert.npz" --out-dir "$covariance/test"
  python -m universal_neuron_adapter.export_covariance_expert \
    --manifest "$SOURCE/$dataset/data/expert_train.csv" \
    --expert-model "$covariance/covariance_expert.npz" \
    --normality-model "$normality/normality_expert.npz" --out-dir "$covariance/train"
  ...
  for baseline in lagovad desc dsanet; do
    ...
    python -m universal_neuron_adapter.evaluate \
      ...
      --expert2-manifest "$covariance/test/expert2_scores.csv" \
      --expert2-train-manifest "$covariance/train/expert2_scores.csv" \
