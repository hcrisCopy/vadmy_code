...
for dataset in ucf xd; do
  ...
  category="$ROOT/category_expert_cache/$dataset/atomic_top16_v1"
  python -m universal_neuron_adapter.fit_category_expert \
    --manifest "$SOURCE/$dataset/data/expert_train.csv" \
    --normality-model "$normality/normality_expert.npz" --out-dir "$category" \
    --active-per-layer 16 --maximum-length 256 --resume
  python -m universal_neuron_adapter.export_category_expert \
    --manifest "$SOURCE/$dataset/data/test.csv" --expert-model "$category/category_expert.npz" \
    --out-dir "$category/test"
  python -m universal_neuron_adapter.export_category_expert \
    --manifest "$SOURCE/$dataset/data/expert_train.csv" --expert-model "$category/category_expert.npz" \
    --out-dir "$category/train"
  ...
  for baseline in lagovad desc dsanet; do
    ...
    python -m universal_neuron_adapter.evaluate \
      ...
      --expert2-manifest "$category/test/expert2_scores.csv" \
      --expert2-train-manifest "$category/train/expert2_scores.csv" \
