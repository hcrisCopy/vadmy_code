...
for dataset in ucf xd; do
  complement="$ROOT/complementary_expert_cache/$dataset/residual_top32_seed234_v1"
  python -m universal_neuron_adapter.train_complementary_expert \
    ...
    --primary-manifest "$SOURCE/$dataset/expert/train/expert_scores.csv" \
    ...
    --standalone-weight 0.25 --maximum-length 256 --num-workers 4 --seed 234 --device cuda --resume
