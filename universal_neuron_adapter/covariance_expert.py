...
def selected_directional_responses(hidden: np.ndarray, normality_model: dict[str, np.ndarray]) -> np.ndarray:
...
def covariance_evidence(hidden: np.ndarray, normality_model: dict[str, np.ndarray], covariance_model: dict[str, np.ndarray]) -> np.ndarray:
    ...
    return (responses * covariance_model["aggregation_weight"][None]).mean(axis=(1, 2)).astype(np.float32)
