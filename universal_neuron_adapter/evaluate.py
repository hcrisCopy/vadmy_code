...
def main() -> None:
    ...
    parser.add_argument("--cross-view-weight", type=float, default=0.5)
    ...
    cross_model = TemporalNeuronHead(int(cross_checkpoint["config"]["input_dim"]), int(cross_checkpoint["config"]["width"]))
    ...
    with torch.no_grad():
        for row in tqdm(frame.itertuples(index=False), total=len(frame), desc=f"evaluate {args.baseline}/{args.dataset}"):
            ...
            selected_neurons = normalize_selected_layers(
                resample_matrix(np.load(str(row.selected_path)), len(base))
            )
            selected_tensor = torch.from_numpy(selected_neurons).unsqueeze(0).to(device)
            cross_curve = torch.sigmoid(cross_model(selected_tensor))[0].cpu().numpy().astype(np.float32)
    ...
    metrics = {
        ...
        "configuration": {
            ...
            "cross_view_neuron_normalization": "within selected neurons of each CLIP layer and snippet",
        },
        "frames": len(truth),
    }
