...
def main() -> None:
    parser = argparse.ArgumentParser(description="Export residual-complement CLS-neuron evidence.")
    ...
    model = SparseNeuronExpert(**checkpoint["config"])
    model.load_state_dict(checkpoint["model_state_dict"])
    model.to(device).eval()
    rows = []
    frame = pd.read_csv(args.manifest)
    with torch.no_grad():
        for row in tqdm(frame.itertuples(index=False), total=len(frame), desc="export residual neuron evidence"):
    ...
    print(f"wrote {len(rows)} residual-complement curves", flush=True)
