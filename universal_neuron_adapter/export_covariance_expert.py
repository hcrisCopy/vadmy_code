...
def main() -> None:
    parser = argparse.ArgumentParser(description="Export covariance-decorrelated neuron evidence.")
    ...
    for row in tqdm(frame.itertuples(index=False), total=len(frame), desc="export decorrelated evidence"):
        target = scores / f"{row.key}.npy"
        if not target.exists():
            np.save(target, covariance_evidence(load_hidden_array(str(row.hidden_path)), normality_model, covariance_model))
