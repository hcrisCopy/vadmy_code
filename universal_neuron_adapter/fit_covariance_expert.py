...
def main() -> None:
    ...
    for row in tqdm(normal.itertuples(index=False), total=len(normal), desc="collect normal co-activations"):
        blocks.append(selected_directional_responses(
            bounded_hidden(str(row.hidden_path), args.maximum_length), normality_model
        ))
    ...
    for layer in tqdm(range(responses.shape[1]), desc="fit layer covariance"):
        ...
        decorrelated /= max(float(decorrelated.mean()), 1e-8)
        aggregation_weights.append(decorrelated)
        shrinkages.append(estimator.shrinkage_)
    np.savez_compressed(
        model_path,
        aggregation_weight=np.asarray(aggregation_weights, dtype=np.float32),
        shrinkage=np.asarray(shrinkages, dtype=np.float32),
    )
    metadata = {
        "definition": "Ledoit-Wolf precision-weighted directional consensus over 32 CLS coordinates per layer",
