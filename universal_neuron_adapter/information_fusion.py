...
def fit_information_fusion_weights(
    ...
    """Fit a training-only regularized Fisher direction over neuron evidence streams."""
    ...
    weights = np.linalg.solve(covariance + ridge * np.eye(len(mean_difference)), mean_difference)
    if not np.isfinite(weights).all() or np.abs(weights).sum() < 1e-8:
        raise ValueError("invalid information-fusion weights")
    weights = weights * (len(weights) / np.abs(weights).sum())
