...
class CrossViewDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> dict[str, object]:
        ...
        if len(neurons) > length:
            positions = np.linspace(0, len(neurons) - 1, length)
            low = np.floor(positions).astype(np.int64)
            high = np.minimum(low + 1, len(neurons) - 1)
            alpha = (positions - low).astype(np.float32)[:, None]
            neurons = neurons[low] * (1.0 - alpha) + neurons[high] * alpha
        ...
        return {"neurons": neurons, "teacher": teacher, "expert": expert, "label": float(row.binary_label)}
...
def collate(batch: list[dict[str, object]]) -> dict[str, torch.Tensor]:
    ...
    width = int(batch[0]["neurons"].shape[1])
    neurons = torch.zeros(len(batch), steps, width)
    teacher = torch.zeros(len(batch), steps)
    ...
    return {
        "neurons": neurons, "teacher": teacher, "expert": expert,
        "labels": torch.tensor([row["label"] for row in batch]), "lengths": lengths,
    }
...
def cross_view_loss(logits: torch.Tensor, batch: dict[str, torch.Tensor]) -> torch.Tensor:
    ...
    for index, length_tensor in enumerate(batch["lengths"]):
        ...
        weight[index, :length][positive] = 2.0
    ...
    bag = F.binary_cross_entropy(torch.stack(bags), batch["labels"])
    return dense + 0.5 * bag


def normalization(dataset: CrossViewDataset) -> tuple[torch.Tensor, torch.Tensor]:
    total = np.zeros(dataset[0]["neurons"].shape[1], dtype=np.float64)
    square = np.zeros_like(total)
    count = 0
    for index in tqdm(range(len(dataset)), desc="cross-view normalization"):
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train a baseline-neuron agreement temporal head.")
    ...
    loader = DataLoader(train, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, collate_fn=collate, generator=torch.Generator().manual_seed(args.seed))
    val_loader = DataLoader(val, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, collate_fn=collate)
    model = TemporalNeuronHead(train[0]["neurons"].shape[1], args.width).to(device)
    ...
    for epoch in range(start_epoch, args.max_epoch):
        model.train()
        total = 0.0
        progress = tqdm(loader, desc=f"cross-view {args.baseline}/{args.dataset} {epoch + 1}/{args.max_epoch}")
        for step, batch in enumerate(progress, 1):
            batch = {key: value.to(device, non_blocking=True) for key, value in batch.items()}
            logits = model((batch["neurons"] - mean_device) / std_device)
            loss = cross_view_loss(logits, batch)
        ...
        with torch.no_grad():
            for batch in val_loader:
                batch = {key: value.to(device, non_blocking=True) for key, value in batch.items()}
                validation += float(cross_view_loss(model((batch["neurons"] - mean_device) / std_device), batch))
        validation /= max(1, len(val_loader))
        payload = {
            "epoch": epoch, "best_metric": min(best, validation),
