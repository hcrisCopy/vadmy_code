...
from universal_neuron_adapter.model import (
    DynamicSparseNeuronExpert,
    SparseNeuronExpert,
    expert_mil_loss,
    topk_bag,
)
...
def evaluate(model: torch.nn.Module, loader: DataLoader, device: torch.device) -> float:
    ...
    targets, scores = [], []
    with torch.no_grad():
        for batch in tqdm(loader, desc="sparse expert validation", leave=False):
            logits = model(batch["hidden"].to(device), batch["lengths"].to(device))
            scores.extend(
                topk_bag(torch.sigmoid(logits), batch["lengths"].to(device)).cpu().tolist()
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train a sparse CLS-neuron expert.")
    parser.add_argument("--train-manifest", required=True)
    parser.add_argument("--val-manifest", required=True)
    parser.add_argument("--out-dir", required=True)
    parser.add_argument("--active-per-layer", type=int, default=64)
    ...
    model_class = DynamicSparseNeuronExpert if args.dynamic else SparseNeuronExpert
    model = model_class(args.active_per_layer, args.temporal_width).to(device)
    ...
    for epoch in range(start_epoch, args.max_epoch):
        model.train()
        total_loss = 0.0
        for batch in tqdm(train_loader, desc=f"sparse expert {epoch + 1}/{args.max_epoch}"):
            logits = model(batch["hidden"].to(device), batch["lengths"].to(device))
        ...
        scheduler.step()
        metric = evaluate(model, validation_loader, device)
        payload = {
            "epoch": epoch,
