...
def main() -> None:
    ...
    if evaluation.count("--expert2-manifest") != 1:
        raise RuntimeError("evaluation must use exactly one shared temporal-information expert")
    if evaluation.count("--expert2-train-manifest") != 1:
        raise RuntimeError("temporal calibration must use exactly one training-only temporal stream")
    ...
    if "train_diverse_expert" in command or "active64_seed" in command:
        raise RuntimeError("the redundant seed-based wide sparse expert is forbidden")
