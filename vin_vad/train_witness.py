...
def fit_role_disentangled_reference(
    ...
    score_sample_weight = torch.cat(normal_score_weights)
    score_threshold = weighted_quantile(normal_score, score_sample_weight, 0.95)
    normalized_weight = score_sample_weight / score_sample_weight.sum()
    score_mean = (normal_score * normalized_weight).sum()
    score_std = torch.sqrt(
        ((normal_score - score_mean).square() * normalized_weight).sum()
    ).clamp_min(1e-2)
    if normal_weight.shape != mean.shape:
        raise RuntimeError("normal-role weights must retain the neuron reference shape")
