...
def context_verification_loss(
    ...
    """Give the context role a distinct weakly supervised verification target."""
    ...
    teacher = (
        masked_standardize(primary, validity)
    ...
    if normal.any():
        class_losses.append(
            masked_mean(-torch.log1p(-context), validity)[normal].mean()
        )
    abnormal_losses = []
    for curve, score, mask, label in zip(context, teacher, validity, labels):
        ...
        valid_curve = curve[mask]
        count = max(1, math.ceil(0.05 * valid_score.numel()))
        indices = torch.topk(valid_score, count).indices
        abnormal_losses.append(-torch.log(valid_curve[indices]).mean())
    if abnormal_losses:
...
def witness_objective(
    ...
    role_losses.append(
        context_verification_loss(result, validity, labels)
        + smooth_weight * temporal_smoothness(
            result["context_evidence"], validity
        )
    )
