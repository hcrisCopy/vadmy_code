...
def masked_temporal_mean(
    ...
    kernel = torch.ones(
        channels, 1, width, dtype=values.dtype, device=values.device
    )
    numerator = F.conv1d(
        values.transpose(1, 2) * mask,
        kernel,
        padding=width // 2,
        groups=channels,
    )
    denominator = F.conv1d(
        mask, kernel[:1], padding=width // 2
    ).clamp_min(1.0)
...
class WitnessExpert(nn.Module):
    ...
    def __init__(self, active: int = 32, temporal_width: int = 64) -> None:
        ...
        self.context_temporal = WitnessTemporalReadout(
            input_channels=24, width=temporal_width
        )
    ...
    def forward(
        ...
        normality_logits = masked_standardize(
            normality_layers.mean(dim=-1), validity
        )
