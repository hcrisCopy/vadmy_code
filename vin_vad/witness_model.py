...
def masked_gaussian_blur(
    ...
    positions = torch.arange(-radius, radius + 1, dtype=values.dtype, device=values.device)
    base = torch.exp(-0.5 * (positions / sigma).square()); base = base / base.sum()
    channels = values.shape[-1]
    kernel = base.view(1, 1, -1).expand(channels, 1, -1)
    mask = validity[:, None].to(values.dtype)
    numerator = F.conv1d(values.transpose(1, 2) * mask, kernel, padding=radius, groups=channels)
    denominator = F.conv1d(mask, base.view(1, 1, -1), padding=radius).clamp_min(1e-6)
    return (numerator / denominator).transpose(1, 2).masked_fill(~validity.unsqueeze(-1), 0.0)
...
class WitnessExpert(nn.Module):
    ...
    def __init__(self, active: int = 32, temporal_width: int = 64) -> None:
        ...
        self.context_temporal = nn.Linear(3 * 12 * active, 1)
    ...
    def forward(
        ...
        context_logits = self.context_temporal(context_input).squeeze(-1).masked_fill(~validity, 0.0)
