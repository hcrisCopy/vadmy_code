...
class WitnessExpert(nn.Module):
    ...
    def forward(
        ...
        context_input = torch.cat(
            [
                masked_temporal_mean(signed_layers, validity, width=9),
                masked_temporal_mean(signed_layers, validity, width=25),
            ],
            dim=-1,
        )
        ...
        # One witness chain, not a jury of loosely motivated roles: the same
        # signed counterfactual departure is read at the current snippet and
        # through its learned temporal context. Their symmetric mean is the
        # relative localization evidence; the normal-calibrated absolute score
        # is passed separately to preserve cross-video ordering.
